...
impl<'a> SpelEvaluator<'a> {
    ...
    pub fn eval(&self, expr: &Expr) -> Result<Value, String> {
        match expr {
            ...
            Expr::PropertyPlaceholder { key, default } => {
                match self.env.get(key.as_str()) {
    ...
    fn eval_binary(&self, op: BinaryOp, left: &Expr, right: &Expr) -> Result<Value, String> {
        ...
        match (&lv, &rv) {
            ...
            (Value::Str(s), Value::Int(b)) => {
                if let Ok(i) = s.parse::<i64>() { return self.int_op(op, i, *b); }
                if let Ok(f) = s.parse::<f64>() { return self.float_op(op, f, *b as f64); }
                if op == BinaryOp::Add { return Ok(Value::Str(format!("{}{}", s, b))); }
                Err(format!("cannot apply {:?} between '{}' (string) and int", op, s))
            }
            (Value::Int(a), Value::Str(s)) => {
                if let Ok(i) = s.parse::<i64>() { return self.int_op(op, *a, i); }
                if let Ok(f) = s.parse::<f64>() { return self.float_op(op, *a as f64, f); }
                if op == BinaryOp::Add { return Ok(Value::Str(format!("{}{}", a, s))); }
                Err(format!("cannot apply {:?} between int and '{}' (string)", op, s))
            }
            (Value::Str(s), Value::Float(b)) => {
                if let Ok(f) = s.parse::<f64>() { return self.float_op(op, f, *b); }
                Err(format!("cannot apply {:?} between '{}' (string) and float", op, s))
            }
            (Value::Float(a), Value::Str(s)) => {
                if let Ok(f) = s.parse::<f64>() { return self.float_op(op, *a, f); }
                Err(format!("cannot apply {:?} between float and '{}' (string)", op, s))
            }
    ...
    fn eval_method(&self, target: Value, method: &str, args: &[Expr]) -> Result<Value, String> {
        match &target {
            ...
            other => Err(format!("cannot call method '{}' on {}", method, other.type_name())),
        }
    }
