...
impl SpelParser {
    ...
    fn parse_comparison(&mut self) -> Result<Expr, String> {
        ...
        let op = match (self.peek(), self.peek_at(1)) {
            (Some('='), Some('=')) => { self.advance(); self.advance(); BinaryOp::Eq }
            (Some('!'), Some('=')) => { self.advance(); self.advance(); BinaryOp::Ne }
            (Some('<'), Some('=')) => { self.advance(); self.advance(); BinaryOp::Le }
            (Some('>'), Some('=')) => { self.advance(); self.advance(); BinaryOp::Ge }
            (Some('<'), _)         => { self.advance(); BinaryOp::Lt }
            (Some('>'), _)         => { self.advance(); BinaryOp::Gt }
            _                      => return Ok(left),
        };
        self.skip_ws();
    ...
    fn parse_addition(&mut self) -> Result<Expr, String> {
        ...
        loop {
            self.skip_ws();
            match self.peek() {
                Some('+') => { self.advance(); self.skip_ws(); let r = self.parse_multiplication()?; left = Expr::Binary { op: BinaryOp::Add, left: Box::new(left), right: Box::new(r) }; }
                Some('-') => { self.advance(); self.skip_ws(); let r = self.parse_multiplication()?; left = Expr::Binary { op: BinaryOp::Sub, left: Box::new(left), right: Box::new(r) }; }
                _ => break,
            }
        }
        Ok(left)
    ...
    fn parse_multiplication(&mut self) -> Result<Expr, String> {
        ...
        loop {
            self.skip_ws();
            match self.peek() {
                Some('*') => { self.advance(); self.skip_ws(); let r = self.parse_unary()?; left = Expr::Binary { op: BinaryOp::Mul, left: Box::new(left), right: Box::new(r) }; }
                Some('/') => { self.advance(); self.skip_ws(); let r = self.parse_unary()?; left = Expr::Binary { op: BinaryOp::Div, left: Box::new(left), right: Box::new(r) }; }
                Some('%') => { self.advance(); self.skip_ws(); let r = self.parse_unary()?; left = Expr::Binary { op: BinaryOp::Rem, left: Box::new(left), right: Box::new(r) }; }
                _ => break,
            }
        }
        Ok(left)
    ...
    fn parse_postfix(&mut self) -> Result<Expr, String> {
        ...
        loop {
            ...
            if self.peek() == Some('.') {
                ...
                self.expect(')')?;
                node = Expr::MethodCall { target: Box::new(node), method, args };
            } else {
                break;
            }
        }
    ...
    fn parse_placeholder(&mut self) -> Result<Expr, String> {
        ...
        self.expect('}')?;
        Ok(Expr::PropertyPlaceholder { key: key.trim().to_string(), default })
    }
    ...
    fn parse_args(&mut self) -> Result<Vec<Expr>, String> {
        self.skip_ws();
        if self.peek() == Some(')') { return Ok(vec![]); }
    ...
    fn peek2(&self) -> Option<[char; 2]> {
        Some([self.chars.get(self.pos).copied()?, self.chars.get(self.pos + 1).copied()?])
    }
    ...
    fn expect(&mut self, c: char) -> Result<(), String> {
        match self.peek() {
            Some(got) if got == c => { self.advance(); Ok(()) }
            got => Err(format!("expected '{}', got {:?} at pos {}", c, got, self.pos)),
        }
    }
}
